...
FROM delta.`s3a://chu/delta/fait_hospitalisation_partitionnee`
WHERE annee BETWEEN 2020 AND 2023
GROUP BY annee;
...
FROM delta.`s3a://chu/delta/fait_hospitalisation_partitionnee` h
JOIN delta.`s3a://chu/delta/dim_patient_partitionnee` p ON h.fk_patient = p.patient_id
...
SELECT
    l.region,
    COUNT(*) AS nb_deces
FROM delta.`s3a://chu/delta/fait_deces_partitionnee` d
JOIN delta.`s3a://chu/delta/dim_lieu_partitionnee` l ON d.fk_lieu_deces = l.lieu_id
WHERE annee = 2019
GROUP BY l.region;
...
FROM delta.`s3a://chu/delta/fait_satisfaction_partitionnee` s
JOIN delta.`s3a://chu/delta/dim_etablissement_partitionnee` e ON s.fk_etablissement = e.etablissement_id
WHERE s.annee = 2020
GROUP BY e.region;